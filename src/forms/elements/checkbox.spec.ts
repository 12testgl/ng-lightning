...
describe('`NglFormCheckbox`', () => {
  ...
  it('should render correctly', testAsync((fixture: ComponentFixture<TestComponent>) => {
    ...
    const checkboxLabelEl = getLabelElement(element);
    expect(checkboxLabelEl).toHaveCssClass('slds-checkbox');

    expect(element.querySelector('input[type=checkbox]')).not.toBe(null);
  }));
